...
$locationId = isset($_POST['id']) ? intval($_POST['id']) : (isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0);
...
// Check if location has assigned departments
...
// No departments assigned, safe to delete
...
if ($deleteQuery->affected_rows === 0) {
    ...
    $output['status']['description'] = "Location not found or already deleted";
    $output['status']['returnedIn'] = (microtime(true) - $executionStartTime) / 1000 . " ms";
    $output['data'] = [];
    $deleteQuery->close();
    mysqli_close($conn);
